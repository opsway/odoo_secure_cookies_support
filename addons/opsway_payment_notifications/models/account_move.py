...
class AccountMove(models.Model):
    ...
    def _send_payment_notification(self):
        ...
        try:
            ...
            _logger.info(
                f"Payment notification sent for move {self.name} to {len(users_to_notify)} users"
            )
            
        except Exception as e:
            _logger.error(f"Error sending payment notification for move {self.name}: {str(e)}")

...
class TestPEEmployee(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.partner_ua.update_field_translations(
            'name', {lang.code: 'Тестовий Партнер'})
        cls.partner_ua.update_field_translations(
            'street', {lang.code: 'Тестова Вулиця'})
        cls.partner_ua.update_field_translations(
            'city', {lang.code: 'Тестове Місто'})
        ...
        cls.env.company.partner_id.update_field_translations(
            'name', {lang.code: 'Тестова Компанія'})
        cls.env.company.partner_id.update_field_translations(
            'street', {lang.code: 'Тестова Вулиця'})
        cls.env.company.partner_id.update_field_translations(
            'city', {lang.code: 'Тестове Місто'})
        cls.env.company.partner_id.update_field_translations(
            'street2', {lang.code: 'Назва Компанії'})
        cls.env.company.default_terms = 'Default Terms and Conditions in English'
        cls.env.company.update_field_translations(
            'default_terms', {lang.code: 'Типові умови українською'})
        cls.partner_en = cls.env['res.partner'].create(
            {'name': 'Test Partner EN', 'lang': 'en_US'})
        cls.product_tag_pe = cls.env['product.tag'].create(
            {'name': 'Personal Employee'})
        cls.product_tag_pe.update_field_translations(
            'name', {lang.code: 'ФОП'})
        ...
        cls.product.update_field_translations(
            'name', {lang.code: 'Тестовий Продукт'})
    ...
    def test_employee_bill_sign_creation(self):
        ...
        self.assertEqual(bill_line.description, self.product.with_context(
            lang=self.lang.code).product_tag_ids[0].name)
    ...
    def test_biilang_employee_bill_report(self):
        ...
        self.assertIn(
            b'Test Partner', report_en[0], "English content not found in English report")
        self.assertIn('Тестовий Партнер'.encode('utf-8'), report_en[0],
                      "Ukrainian content not found in Ukrainian report")
        self.assertIn(
            b'Test Street', report_en[0], "English address not found in English report")
        self.assertIn('Тестова Вулиця'.encode(
            'utf-8'), report_en[0], "Ukrainian address not found in Ukrainian report")
        self.assertIn(
            b'Test City', report_en[0], "English city not found in English report")
        self.assertIn('Тестове Місто'.encode(
            'utf-8'), report_en[0], "Ukrainian city not found in Ukrainian report")
        self.assertIn(
            b'12345', report_en[0], "English zip not found in English report")
        self.assertIn('12345'.encode(
            'utf-8'), report_en[0], "Ukrainian zip not found in Ukrainian report")
        # check company data in report
        self.assertIn(
            b'Test Company', report_en[0], "English company name not found in English report")
        self.assertIn('Тестова Компанія'.encode('utf-8'), report_en[0],
                      "Ukrainian company name not found in Ukrainian report")
        self.assertIn(
            b'Test Street', report_en[0], "English company street not found in English report")
        self.assertIn('Тестова Вулиця'.encode('utf-8'), report_en[0],
                      "Ukrainian company street not found in Ukrainian report")
        self.assertIn(b'Name of Company',
                      report_en[0], "English company street2 not found in English report")
        self.assertIn('Назва Компанії'.encode('utf-8'), report_en[0],
                      "Ukrainian company street2 not found in Ukrainian report")
        self.assertIn(b'Personal Employee',
                      report_en[0], "English product name not found in English report")
        ...
        self.assertIn(b'May 15, 2024',
                      report_en[0], "Invoice date not found in report")
        self.assertIn(b'2530.58', report_en[0],
                      "Total amount not found in report")
        ...
        self.assertIn('Total: Дві тисячі п&#39;ятсот тридцять dollars і п&#39;ятдесят вісім cents'.encode('utf-8'),
                      report_en[0],
                      "Total amount in words not found in report")
        # check bank account in report
        self.assertIn(
            b'123456789', report_en[0], "Bank account not found in report")
        self.assertIn(
            b'Test Bank', report_en[0], "Bank name not found in report")
        self.assertIn(
            b'123456789', report_en[0], "Bank BIC not found in report")
    ...
    def test_get_vendor_invoice_date(self):
        ...
        self.assertEqual(
            self.invoice_ua._get_vendor_invoice_date(), expected_date)

    def test_get_vendor_invoice_period(self):
        """Test _get_vendor_invoice_period method."""
        expected_period = "May 2024"
        self.assertEqual(
            self.invoice_ua._get_vendor_invoice_period(), expected_period)
    ...
    def test_get_total_amount_in_word_pe(self):
        ...
        self.assertEqual(
            self.invoice_ua._get_total_amount_in_word_pe(), expected_text)
    ...
    def test_get_pe_report_filename(self):
        ...
        self.assertEqual(
            self.invoice_ua._get_pe_report_filename(), expected_filename)

    def test_get_report_description(self):
        """Test _get_report_description method."""
        self.invoice_ua.action_post()
        self.assertTrue(len(self.invoice_ua.employee_bill_sign_ids)
                        == 1, "Bill Sign lines not created.")
        self.assertEqual(self.invoice_ua.employee_bill_sign_ids[0].description, "ФОП",
                         "Report description is incorrect.")
        self.assertEqual(
            self.invoice_ua.with_context(
                lang=self.lang.code).employee_bill_sign_ids[0]._get_report_description(),
